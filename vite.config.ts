...
const resolveDevServerPort = async (): Promise<number> => {
  ...
  // On Windows/WSL, certain ports can be reserved/excluded and throw EACCES.
  // Vite only auto-increments for EADDRINUSE, so we preflight a few candidates.
  const candidates = [
  ...
  for (const port of candidates) {
    // eslint-disable-next-line no-await-in-loop -- sequential probe avoids race on port binding
...
export default defineConfig(async ({ command }) => {
  ...
  return {
    ...
    server: {
      host: DEV_SERVER_HOST, // Force IPv4 to avoid Windows EACCES on ::1
      port: devServerPort,
    },
  }
})
