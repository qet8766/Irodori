...
export const getKeyboardWorker = (): { worker: ReturnType<typeof spawn>; ready: Promise<void> } => {
...
export const sendKeyboardCommand = async (cmd: 'R' | 'C' | 'V'): Promise<string> => {
  const { worker, ready } = getKeyboardWorker()
  await ready
